...
public class CheckInServiceImpl implements CheckInService {
    ...
    public ResponseResult<CheckInVo> createCheckIn(CheckInDto checkInDto) {
        ...
        if (checkIn.getId() != null) {
            ...
            JSONObject jsonObjectOld = JSONUtil.parseObj(checkIn1.getOtherApplyInfo());
            String elderDtostrOld = jsonObjectOld.getStr("elderDto");
            ElderDto elderDtoOld = JSONUtil.toBean(elderDtostrOld, ElderDto.class);
            ...
            String elderDtostr = jsonObject.getStr("elderDto");
            ElderDto elderDto1 = JSONUtil.toBean(elderDtostr, ElderDto.class);
    ...
    public ResponseResult<TasVo> getCheckIn(String code, String assigneeId, Integer flowStatus, String taskId) {
        ...
        JSONObject jsonObject = JSONUtil.parseObj(checkInVo.getOtherApplyInfo());
        if (ObjectUtil.isNotEmpty(jsonObject)) {
            String s = jsonObject.getStr("checkInOtherDto");
            CheckInOtherDto checkInOtherDto = JSONUtil.toBean(s, CheckInOtherDto.class);
            ...
            String elderDto = jsonObject.getStr("elderDto");
            ElderDto elderDto1 = JSONUtil.toBean(elderDto, ElderDto.class);
            checkInVo.setElderDto(elderDto1);
        }
        //查询入住评估内容
        JSONObject review = JSONUtil.parseObj(checkInVo.getReviewInfo());
        if (ObjectUtil.isNotEmpty(review)) {
            String r1 = review.getStr("r1");
            String r2 = review.getStr("r2");
...
//        accraditationRecordService.insert(user.getId(), user, checkIn, AccraditationRecordConstant.AUDIT_STATUS_WITHDRAWS, "撤回", "撤回申请-" + name, "");
...
//        accraditationRecordService.insert(nextAssignee, user, checkIn, AccraditationRecordConstant.AUDIT_STATUS_PASS, "同意", "发起申请-申请入住", "护理组组长处理-入住评估");
    ...
    public ResponseResult<CheckInVo> cancel(Long id, String taskId) {
        ...
        //记录操作记录
//        accraditationRecordService.insert(null, user, checkIn, AccraditationRecordConstant.AUDIT_STATUS_CANCEL, "撤销", "撤销申请-入住申请", "");
    ...
    public ResponseResult<CheckInVo> review(CheckInDto checkInDto) {
        ...
        JSONObject jsonObject = JSONUtil.parseObj(checkInDto.getReviewInfo());
...
//        accraditationRecordService.insert(nextAssignee, user, checkIn, AccraditationRecordConstant.AUDIT_STATUS_PASS, "同意", "护理部组长处理-入住评估", "院长处理-入住审批");
